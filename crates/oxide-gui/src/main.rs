...
pub struct AppCompresseur {
    ...
    // NOUVEAU : Champs pour la sécurité du mot de passe
    mot_de_passe: String,
    confirmation_mot_de_passe: String, 
    pourcentage_protection: u8,
    
    // Sécurité pour éviter les crashs de l'explorateur (Linux/Wayland)
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            egui::ScrollArea::vertical().show(ui, |ui| {
                ...
                // --- NOUVEAU : GESTION DU MOT DE PASSE AVEC CONFIRMATION ---
                        ...
                        let resultat = match mode {
                            ...
                            Mode::Verifier => {
                                let mut verifier_archive = || -> Result<String, String> {
                                    ...
                                    let mut erreurs = 0;
                                    let mut dernier_rafraichissement = Instant::now(); // Correctif de limite de vitesse pour éviter de crasher l'interface
