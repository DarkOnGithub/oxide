...
mod tests {
    ...
    fn round_trip_case(data: &[u8]) {
        let encoded = apply(data).expect("compress should succeed");
        let decoded = reverse(&encoded).expect("decompress should succeed");
    ...
    fn decode_legacy_fixture_vectors() {
        ...
        for (encoded_hex, expected) in fixtures {
            ...
            let decoded = reverse(&encoded).expect("legacy vector must decode");
    ...
    fn decode_rejects_short_prefix() {
        assert!(reverse(&[1, 2, 3]).is_err());
    }

    #[test]
    fn decode_rejects_truncated_stream() {
        let mut encoded = apply(b"this is a test payload").expect("compress should succeed");
        encoded.pop();
