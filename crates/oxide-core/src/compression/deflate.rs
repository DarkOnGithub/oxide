...
#[cfg(test)]
mod tests {
    use super::{apply, reverse};

    fn roundtrip(data: &[u8]) {
        let encoded = apply(data).expect("encode should succeed");
        let decoded = reverse(&encoded).expect("decode should succeed");
    ...
    fn deterministic_encoding() {
        ...
        let a = apply(data).expect("encode");
        let b = apply(data).expect("encode");
        assert_eq!(a, b);
    }

    #[test]
    fn rejects_invalid_magic() {
        let mut encoded = apply(b"abc").expect("encode");
        encoded[4] ^= 0xFF;
        assert!(reverse(&encoded).is_err());
    }

    #[test]
    fn rejects_truncated_stream() {
        let mut encoded = apply(b"this is a payload").expect("encode");
        encoded.pop();
        assert!(reverse(&encoded).is_err());
    }
    ...
    fn rejects_trailing_garbage() {
        let mut encoded = apply(b"abcabcabcabcabc").expect("encode");
        encoded.push(0xFF);
        assert!(reverse(&encoded).is_err());
    }
}
