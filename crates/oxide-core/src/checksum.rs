/// Computes the archive checksum for the provided bytes.
use std::arch::is_x86_feature_detected;

/// Computes CRC32C (Castagnoli) for the provided bytes.
#[inline]
pub fn compute_checksum(bytes: &[u8]) -> u32 {
    // 1. Try x86_64 Hardware Acceleration
    ...
    // 2. Fallback to Optimized Software Implementation
    crc32c_sw(bytes)
}
...
unsafe fn crc32c_hw_x86_64(bytes: &[u8]) -> u32 {
    ...
    // Process blocks of 8 bytes
    while i + 8 <= len {
        let chunk = (bytes.as_ptr().add(i) as *const u64).read_unaligned();
        crc = _mm_crc32_u64(crc, chunk);
        i += 8;
    }

    // Process remaining blocks of 4 bytes
    while i + 4 <= len {
        let chunk = (bytes.as_ptr().add(i) as *const u32).read_unaligned();
...
// --- Optimized Software Implementation (Slice-by-1) ---
