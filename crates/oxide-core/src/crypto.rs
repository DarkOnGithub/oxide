...
pub fn decrypt_block(key: &[u8; KEY_SIZE], payload: &[u8]) -> Result<Vec<u8>> {
    if payload.len() < NONCE_SIZE {
        return Err(OxideError::InvalidFormat("encrypted block is too short to contain a nonce"));
    }
    ...
    let plaintext = cipher.decrypt(nonce, ciphertext)
        .map_err(|_| OxideError::InvalidFormat("decryption failed or data is corrupted (invalid password?)"))?;
